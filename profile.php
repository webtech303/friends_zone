	...
	<div class="col-sm-6">
		...
		while ($row_posts = mysqli_fetch_array($run_posts)) {
			...
			if($content == "no" && strlen($upload_image) >= 1){
				...
				$get_user = "select * from users ehere user_email='$user'";
				$rin_user = mysqli_query($con,$get_user);
				$row = mysqli_fetch_array($run_user);
				...
				if($u_email != $user_email){
					echo "<script>window.open('profile.php?u_id=$user_id','_self')</sript>";
				}else{
					echo"
					<a href='single.php?post_id=$post_id' style='float:right;'><button class='btn btn-success'>View</button></a><br>

					<a href='edit.php?post_id=$post_id' style='float:right;'><button class='btn btn-info'>Edit</button></a><br>
